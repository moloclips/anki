...
DEFAULT_DECK_NAME = "AI Politics Prep"
...
def render_template(template: str, row: dict[str, str]) -> str:
    ...
    def replace(match: re.Match[str]) -> str:
        key = match.group(1)
        return html.escape(format_display_value(key, row.get(key, "")))
        ...
        "basic_and_reversed": {
            ...
            "templates": [
                ...
                },
                {
                    ...
                    "afmt": "{{#Add Reverse}}{{FrontSide}}<hr id=\"answer\">{{Front}}{{/Add Reverse}}",
                },
            ],
            "css": css,
        },
...
def load_notes(config: dict[str, Any]) -> list[NoteRecord]:
    ...
    for note in notes:
        if note.model_kind == "basic_and_reversed":
            reverse_counts[note.back] = reverse_counts.get(note.back, 0) + 1

    normalized_notes: list[NoteRecord] = []
    for note in notes:
        reverse_enabled = note.model_kind != "basic_and_reversed" or reverse_counts.get(note.back, 0) == 1
        normalized_notes.append(
            NoteRecord(
                guid_key=note.guid_key,
                model_kind=note.model_kind,
                front=note.front,
                back=note.back,
        ...
        deck.add_note(
            genanki.Note(
                ...
                fields=(
                    ...
                    else [note.front, note.back, "yes" if note.reverse_enabled else ""]
                ),
...
def build(config_path: Path, deck_name_override: str | None, output_dir: Path) -> tuple[int, Path, Path, bool]:
    ...
    if not notes:
        raise ValueError("No notes found. Add rows to Anki/*.csv or card definitions to deck_config.json.")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build an Anki deck and HTML preview from deck_config.json.")
